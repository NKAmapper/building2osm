...
original = (
    False  # Output polygons as in original data (no rectification/simplification)
)
...
simplify_margin = (
    0.05  # Minimum tolerance for buildings with curves in simplification (meters)
)
...
def load_building_types():
    ...
    for row in building_csv:
        ...
        building_types[row["id"]] = {"name": row["name"], "tags": osm_tag}
...
def load_building_coordinates(municipality_id, min_bbox, max_bbox, level):
    ...
    url = (
        ...
        + "service=WFS&version=2.0.0&request=GetFeature&&srsName=EPSG:4326&typename=Building&bbox="
        + ",".join(bbox_list)
    )
    # 	message ("\n\tQuery: %s\n\t" % url)
    ...
    if count_feature > max_download - 10:
        if verbose:
            message(
                "%s*** Too many buildings in box, force split box and reloading"
                % ("\t" * level)
            )
        count_load += load_area(
            municipality_id, min_bbox, max_bbox, level, force_divide=True
        )
    elif not verbose:
        countdown = len(buildings) - sum(
...
def load_area(municipality_id, min_bbox, max_bbox, level, force_divide):
    ...
    for building in buildings.values():
        if (
            min_bbox[0] <= building["centre"][0] < max_bbox[0]
    ...
    for building_node in neighbour_buildings:
        if (
            min_bbox[0] <= building_node[0] < max_bbox[0]
            and min_bbox[1] <= building_node[1] < max_bbox[1]
        ):
            neighbour_inside_box += 1

    if verbose and not force_divide:
        message(
            "%sExpecting %i buildings + %i neighbours ... "
            % ("\t" * level, inside_box, neighbour_inside_box)
        )
    ...
    # Do actual loading of data
        ...
        count_load += load_building_coordinates(
            municipality_id, min_bbox, max_bbox, level
        )
        ...
        if distance((min_bbox[0], max_bbox[1]), max_bbox) > distance(
            min_bbox, (min_bbox[0], max_bbox[1])
        ):  # x longer than y
            # Split x axis
...
def load_coordinates_municipality(municipality_id):
    ...
    file = urllib.request.urlopen(
        "https://ws.geonorge.no/kommuneinfo/v1/kommuner/" + municipality_id
    )
    data = json.load(file)
    file.close()
    ...
    count_load = load_area(
        municipality_id, bbox[0], bbox[2], 1, force_divide=False
    ...
    for building in buildings.values():
        if (
            ...
            and building["properties"]["building"] in ["garage", "barn", "hotel"]
        ):
    ...
    count_polygons = sum(
        (building["geometry"]["type"] == "Polygon") for building in buildings.values()
    )
    message(
        "\r\tLoaded %i building polygons with %i load queries\n"
...
def load_building_info(municipality_id, municipality_name, neighbour):
    ...
    if len(zip_file.namelist()) == 0:
        if neighbour:
            max_download = (
                0.5 * max_download
            )  # Aim for less aggressive target since neighbour info will be incomplete
            return 0
        else:
            message(
    ...
    for feature in root.iter("{%s}featureMember" % ns_gml):
        ...
        if neighbour:
            neighbour_buildings.append(
                centre
            )  # We only need centre coordinates for neighbour municipalities
            continue
        ...
        if sefrak is not None:
            sefrak = "%s-%s-%s" % (
                sefrak.find("app:sefrakKommune", ns).text,
        ...
        if building_type in building_types:
            feature["properties"]["TYPE"] = "#%s %s" % (
                building_type,
                building_types[building_type]["name"],
            )
        ...
        if debug:
            feature["properties"]["DEBUG_CENTRE"] = (
                str(centre[1]) + " " + str(centre[0])
            )
...
def assign_levels_to_building(main_levels, roof_levels, point):
    ...
    for ref, building in iter(buildings.items()):
        if (
            min_lon < building["centre"][0] < max_lon
            and min_lat < building["centre"][1] < max_lat
        ):
            if (
                building["geometry"]["type"] == "Polygon"
                and building["properties"]["building"]
                in [
                    "apartments",
                    "residential",
                    "dormitory",
                ...
                and inside_polygon(point, building["geometry"]["coordinates"][0])
            ):
                found_ref = ref
                break
    ...
    if found_ref:
        ...
        if "building:levels" in building["properties"] and main_levels != int(
            building["properties"]["building:levels"]
        ):
            building["properties"]["building:levels"] = str(
                max(main_levels, int(building["properties"]["building:levels"]))
            )
            building["properties"]["DEBUG_LEVELS"] = building["properties"][
                "building:levels"
            ]
        else:
        ...
        if roof_levels > 0:
            if "roof:levels" in building["properties"]:
                building["properties"]["roof:levels"] = str(
                    max(roof_levels, int(building["properties"]["roof:levels"]))
                )
            else:
        ...
        if row["adresseId"] != address:
            ...
            if address:
                if levels["H"] + levels["U"] > 1:
                    assign_levels_to_building(
                        levels["H"] + levels["U"], levels["L"], point
                    )
                    count += 1
                    message("\r\t%i " % count)
...
def simplify_buildings():
    ...
    message(
        "\tSimplification factor: %.2f m (curve), %i degrees (line)\n"
    ...
    for ref, building in iter(buildings.items()):
        if building["geometry"]["type"] == "Polygon" and (
            "rectified" not in building or building["rectified"] == "no"
        ):
            ...
            for polygon in building["geometry"]["coordinates"]:
                ...
                for i in range(1, len(polygon) - 1):
                    new_bearing = bearing_turn(
                        polygon[i - 1], polygon[i], polygon[i + 1]
                    )

                    if (
                        math.copysign(1, last_bearing) == math.copysign(1, new_bearing)
                        and curve_margin_min < abs(new_bearing) < curve_margin_max
                    ):
                        curve.add(i - 1)
                        curve.add(i)
                        curve.add(i + 1)
                ...
                if curves:
                    ...
                    if (
                        line_distance(new_polygon[-2], new_polygon[1], new_polygon[0])
                        < simplify_margin
                    ):
                    ...
                    if len(new_polygon) < len(polygon):
                        building["properties"]["VERIFY_SIMPLIFY_CURVE"] = str(
    ...
    message(
        "\tRemoved %i redundant nodes in %i buildings\n"
...
def rectify_buildings():
    ...
    for ref, building in iter(buildings.items()):
        if building["geometry"]["type"] == "Polygon":
            for polygon in building["geometry"]["coordinates"]:
                for node in polygon[:-1]:
                    if node not in nodes:
                        nodes[node] = {"use": 1, "parents": [building]}
                    else:
                        nodes[node]["use"] += 1
    ...
    for ref, building_test in iter(buildings.items()):
        ...
        if (
            building_test["geometry"]["type"] != "Polygon"
        ...
        while check_neighbours:
            for neighbour in check_neighbours[0]["neighbours"]:
                if (
                    neighbour not in building_group
                    and neighbour not in check_neighbours
                ):
        ...
        for building in building_group:
            ...
            for patch, polygon in enumerate(building["geometry"]["coordinates"]):
                ...
                for i in range(len(polygon) - 1):
                    ...
                    short_length = min(
                        distance(last_corner, polygon[i]),
                        distance(polygon[i], polygon[i + 1]),
                    )  # Test short walls

                    # Remove short wall if on (almost) straight line
                    if (
                        distance(polygon[i], polygon[i + 1]) < short_margin
                        and abs(
                            test_corner
                            + bearing_turn(
                                polygon[i],
                        ...
                        building["properties"][
                            "VERIFY_SHORT_REMOVE"
                        ] = "%.2f" % distance(polygon[i], polygon[i + 1])

                    # Identify (almost) 90 degree corner and start new wall
                    elif (
                        90 - angle_margin < abs(test_corner) < 90 + angle_margin
                        or short_length < corner_margin
                        and 60 < abs(test_corner) < 120
                        and nodes[polygon[i]]["use"] == 1
                    ):
                    ...
                    # Else throw away node (redundant node on (almost) straight line)
                        ...
                        update_corner(
                            corners, None, polygon[i], 0
                        )  # Node on "straight" line, will not be used

                    # For debugging, mark cases where a slightly larger margin would have produced a rectified polygon
                    if (
                        count_corners != last_count
                        and not conform
                        and 90 - angle_margin + 2
                        < abs(test_corner)
                        < 90 + angle_margin + 2
                    ):
                        building["properties"]["DEBUG_MISSED_CORNER"] = str(
                            int(abs(test_corner))
                        )
        ...
        avg_bearing = statistics.median_low(
            bearings
        )  # Use median to get dominant bearings

        building["properties"]["DEBUG_BEARINGS"] = str(
            [int(degree) for degree in bearings]
        )
        building["properties"]["DEBUG_AXIS"] = str(
            [wall["axis"] for patch in walls for wall in patch]
        )
        ...
        combine_walls = (
            []
        )  # List will contain all combinations of walls in group which can be combined

        for wall in walls:
            if any(
                wall in w for w in combine_walls
            ):  # Avoid walls which are already combined
                continue
                ...
                if check_neighbours[0]["axis"] == wall["axis"]:
                    for node in check_neighbours[0]["nodes"]:
                        for check_wall in corners[node]["walls"]:
                            if (
                                check_wall["axis"] == wall["axis"]
                                and check_wall not in check_neighbours
                                and check_wall not in connected_walls
                            ):
        ...
        if combine_walls:
            building_test["properties"]["DEBUG_COMBINE"] = str(
                [len(l) for l in combine_walls]
            )
        ...
        for combination in combine_walls:
            ...
            for wall in combination[1:]:
                main_wall["nodes"].extend(
                    list(set(wall["nodes"]) - set(main_wall["nodes"]))
                )
        ...
        for wall in walls:
            ...
            x = statistics.mean(
                [corners[node]["new_node"][0] for node in wall["nodes"]]
            )
            y = statistics.mean(
                [corners[node]["new_node"][1] for node in wall["nodes"]]
            )
        ...
        for node, corner in iter(corners.items()):
            ...
            corner["new_node"] = (
                round(corner["new_node"][0], coordinate_decimals),
                round(corner["new_node"][1], coordinate_decimals),
            )
        ...
        for building in building_group:
            for i, polygon in enumerate(building["geometry"]["coordinates"]):
                for node in polygon:
                    if node in corners:
                        relocated = max(
                            relocated, distance(node, corners[node]["new_node"])
                        )
        ...
        if relocated < rectify_margin:
            ...
            for building in building_group:
                ...
                for i, polygon in enumerate(building["geometry"]["coordinates"]):
                    ...
                    if (
                        new_polygon[0] != new_polygon[-1]
                    ):  # First + last node were removed
            ...
            building_test["properties"]["DEBUG_NORECTIFY"] = (
                "Node relocated %.1f m" % relocated
            )
            for building in building_group:
                building["rectified"] = "no"  # Do not test again
        ...
        if building["geometry"]["coordinates"]:
            ...
            if not debug:
                for key in list(building["properties"].keys()):
                    if (
                        key == key.upper()
                        and key not in ["TYPE", "STATUS", "DATE"]
                        and not (verify and "VERIFY" in key)
                        and not (original and key == "SEFRAK")
                    ):
...
if __name__ == "__main__":
    ...
    if len(municipality_id) == 2:  # County
        message(
            "Generating building files for all municipalities in %s\n\n"
            % municipalities[municipality_id]
        )
        for mun_id in municipalities:
            if (
                len(mun_id) == 4
                and mun_id[0:2] == municipality_id
                or municipality_id == "00"
            ):
