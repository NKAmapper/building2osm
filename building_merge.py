...
def load_import_buildings(file_name):
    ...
    for building in import_buildings:
        if (
            ...
            and len(building["geometry"]["coordinates"]) == 1
        ):
        ...
        if (
            building["properties"]["building"] == "hotel"
            and "area" in building
            and building["area"] < 100
        ):
            building["properties"]["building"] = "cabin"
        if (
            building["properties"]["building"] in ["garage", "barn"]
            and "area" in building
            and building["area"] < 15
        ):
    ...
    for element in osm_elements:
        if element["type"] == "node":
            ...
            if (
                ...
                and "building:part" not in element["tags"]
                and len(element["nodes"]) > 2
                and element["nodes"][0] == element["nodes"][-1]
                and not element["id"] in relation_members
            ):
    ...
    for building in osm_buildings:
        ...
        if building["type"] == "way":
            ...
            for node_ref in building["nodes"]:
                if node_ref in osm_nodes:
                    line.append(
                        (osm_nodes[node_ref]["lon"], osm_nodes[node_ref]["lat"])
                    )
                    # 				if node_ref in osm_nodes:
                    osm_nodes[node_ref]["used"] += 1
...
def add_building(building, osm_element):
    ...
    # Simple polygon
        ...
        if (
            osm_element is not None
            and way_element["tags"]["building"] != "yes"
            and way_element["tags"]["building"] != building["properties"]["building"]
            and not (
                way_element["tags"]["building"] in similar_buildings["residential"]
                and building["properties"]["building"]
                in similar_buildings["residential"]
            )
            and not (
                way_element["tags"]["building"] in similar_buildings["commercial"]
                and building["properties"]["building"]
        ...
        for tag in (
            ["building:type", "source", "source:date"]
            or remove_addr
            and tag
            in [
                "addr:street",
    ...
    # Multipolygon
        ...
        for patch in building["geometry"]["coordinates"]:
            ...
            member = {"type": "way", "ref": way_element["id"], "role": role}
...
def merge_buildings():
        ...
        if found_building is not None:
            ...
            if (
                ...
                if found_reverse == osm_building and reverse_hausdorff < margin_hausdorff:
                    ...
                    if (
                        margin_area
                        < osm_building["area"] / found_building["area"]
                        < 1.0 / margin_area
                    ):
    ...
    message(
        "\r\tMerged %i buildings from OSM (%i%%)\n"
        % (count_merge, 100.0 * count_merge / len(osm_buildings))
    )
    message(
        "\tRemaining %i buildings from OSM not merged (%i%%)\n"
        % (
            len(osm_buildings) - count_merge,
            100 - 100.0 * count_merge / len(osm_buildings),
        )
    )
