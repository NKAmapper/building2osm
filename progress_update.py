...
buildings_per_second = (
    10000  # Number of buildings per 1 second sleep time before Overpass requests
)
    ...
    while tries < 5:
        try:
            ...
            if e.code in [
                ...
                503,
                504,
            ]:  # Too many requests, Service unavailable or Gateway timed out
                if tries == 0:
                    message("\n")
...
def load_progress_page():
    ...
    table = content.find(
        "caption", text="Import progress table - Municipalities\n"
    ).find_parent("table")
    ...
    municipality_ids = {
        municipality["name"]: municipality_id
        for municipality_id, municipality in municipalities.items()
    }

    table = content.find(
        "caption", text="Import progress table - Bydeler\n"
    ).find_parent("table")
...
def count_osm_buildings():
    ...
    for municipality_id, municipality in municipalities.items():
        ...
        try:
            municipality["ref_progress"] = int(
                100 * count_tags / municipality["import_buildings"]
            )
        ...
        for subdivision in municipality.get("subdivision", []):
            ...
            count_tags = int(data["elements"][0]["tags"]["total"])
            try:
                subdivision["ref_progress"] = int(
                    100 * count_tags / subdivision["import_buildings"]
                )
            except ZeroDivisionError:
    ...
    municipalities[norway_id]["ref_progress"] = int(
        100 * total_tags / municipalities[norway_id]["import_buildings"]
    )
        ...
        for municipality_id, municipality in municipalities.items():
            ...
            if municipality_id != norway_id:
                ...
                ref_count += (
                    ...
                    * municipality["import_buildings"]
                    / 100.0
                )

        ref_count = int(100.0 * ref_count / import_count)
        message(
            f"\t{'Total in Norway':<41} {import_count:6d} {osm_count:6d} {ref_count:d}%\n\n"
        )
